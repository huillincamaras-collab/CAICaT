...
Flujo:
  1. scan_batch_folder() → detecta estructura de carpetas
  2. create_batch_manifest() → crea manifest con metadata por archivo
  3. process_batch_videos() → procesa todo (videos + fotos huérfanas en modo normal)
  4. apply_metadata_to_batch() → asigna site/camera/operator por carpeta
  5. create_batch_session_metadata() → genera metadata.json para el tagger
"""
import os
import json
import shutil
...
def scan_batch_folder(root_folder):
    ...
    Detecta tanto videos como fotos huérfanas.
...
def create_batch_manifest(batch_id, folder_structure, output_folder, use_legacy=False, root_folder=""):
    ...
    manifest = {
        ...
        "root_folder": root_folder,  # 🔒 Guardar carpeta raíz
        "folders": {}
    }
    ...
    for folder_key, folder_data in folder_structure.items():
        manifest["folders"][folder_key] = {
            ...
            "files": [],  # Se llena durante el procesamiento
            "error_files": []  # 🔒 Trackeo de errores por archivo
        }
...
def process_batch_videos(folder_structure, output_folder, manifest_path,
    ...
    """
    Procesa todos los videos y fotos del batch.
    - NO mueve carpetas de frames (usa nomenclatura directa)
    - Guarda progreso al final de cada carpeta en el manifest
    - Procesa videos + fotos huérfanas (solo en modo normal)
    - Preserva video_path original
    - Trackea errores por archivo individual
    ...
    for folder_idx, (folder_key, folder_data) in enumerate(folder_structure.items(), 1):
        ...
        try:
            ...
            # Escanear y procesar
            if use_legacy:
                # 🔒 MODO LEGACY: Solo videos (NO procesa fotos huérfanas)
                metadata_list = escanear_videos_legacy(folder_data["path"], temp_output)
                ...
                # MODO NORMAL: Videos + fotos huérfanas
                metadata_list = escanear_videos(folder_data["path"], temp_output)
                processed = []
            ...
            # Mover carpetas de frames a batch/ con nomenclatura correcta
            # Y registrar cada archivo en el manifest
            files_info = []
            error_files = []  # 🔒 Trackeo de errores
            ...
            for meta in processed:
                ...
                # Registrar info del archivo en el manifest
                file_info = {
                    "video_path": meta.get("video_path", ""),
                    "video_hash": video_hash,
            ...
            # Actualizar manifest con los archivos procesados
            manifest["folders"][folder_key]["files"] = files_info
            manifest["folders"][folder_key]["error_files"] = error_files  # 🔒 Guardar errores
            ...
            # Guardar progreso al final de cada carpeta
            _save_manifest(manifest, manifest_path)
            
            if error_files:
                print(f"[Batch]   ⚠️ Carpeta completada con {len(error_files)} errores")
            else:
                print(f"[Batch]   ✅ Carpeta completada: {len(files_info)} archivos procesados")
...
def resume_batch_processing(manifest_path, output_folder, progress_callback=None):
    ...
    Args:
        manifest_path: Ruta al batch_manifest.json
        output_folder: Carpeta de salida
        progress_callback: Función opcional
...
def apply_metadata_to_batch(manifest_path, folder_metadata_dict):
    ...
    Args:
        manifest_path: Ruta al batch_manifest.json
        folder_metadata_dict: Dict con metadata por carpeta:
            {
                "folder_key": {"site": "X", "subsite": "Y", "camera": "Z", "operator": "W"}
            }
...
def load_batch_metadata(batch_folder):
    ...
    Args:
        batch_folder: Carpeta batch/
        
    Returns:
        Lista de metadata de todos los videos (estructura idéntica a la de gui_inicial)
    """
    manifest_path = os.path.join(batch_folder, "batch_manifest.json")
    ...
    for folder_key, folder_info in manifest["folders"].items():
        ...
        for file_info in folder_info.get("files", []):
            # Construir entrada de metadata con estructura idéntica a gui_inicial
            video_meta = {
                ...
                "session": {
                    "session_id": "",  # Se asigna al crear la sesión
                    "camtrap_db_session": False,
                    "deployment_id": ""
                }
            }
            
            # Agregar dict "file" anidado (modelo nuevo)
            video_meta["file"] = {
...
def create_batch_session_metadata(manifest_path, output_folder, session_id):
    ...
    Usa los datos del manifest (video_path, recorded_at, etc.) y aplica metadata
    por carpeta.
    ...
    Returns:
        Ruta al metadata.json creado
    """
    manifest = load_manifest(manifest_path)
    
    # Generar deployment_id base
    all_videos = []
    ...
    for folder_key, folder_info in manifest["folders"].items():
        ...
        # Generar deployment_id para esta carpeta
        ...
        for file_info in folder_info.get("files", []):
            ...
            # Dict "file" anidado (modelo nuevo)
            video_meta["file"] = {
                "video_path": video_meta["video_path"],
                "video_hash": video_meta["video_hash"],
                "frames_folder": video_meta["frames_folder"],
    ...
    # Ordenar por recorded_at (timestamp)
    ...
    # Actualizar manifest con la sesión creada
    manifest["session_id"] = session_id
    manifest["status"] = "ready_for_tagging"
    _save_manifest(manifest, manifest_path)
    
    print(f"[Batch] ✅ Sesión creada: {metadata_path}")
    print(f"[Batch]    {len(all_videos)} videos/fotos listos para etiquetar")
...
def discard_batch(output_folder):
    """
    Descarta completamente un batch (elimina carpeta batch/ y sesión asociada).
    ...
    try:
        # Cargar manifest para obtener session_id
        manifest = load_manifest(manifest_path)
        ...
        # Eliminar carpeta batch completa
